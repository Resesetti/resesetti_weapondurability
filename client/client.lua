DegradeHook = {
	...
	getWeapon = {},
	resesettiDegradeHandler = {},
}
DegradeHook.__index = DegradeHook

function DegradeHook:new()
	local this = setmetatable({}, DegradeHook)
	self.getWeapon = function(hash)
		return self.weaponTab[hash] or false
	end
	self.resesettiDegradeHandler = function()
	AddEventHandler('gameEventTriggered',function (event,data)
		if event=="CEventNetworkEntityDamage" then
			local attacker, weaponHash = data[2], data[7]
			if self.getWeapon(weaponHash) == "WEAPON_UNARMED" then return end 
			local weaponGroup = GetWeapontypeGroup(weaponHash)
			local weaponName = self.getWeapon(weaponHash)
			attacker =  IsPedAPlayer(attacker) and GetPlayerServerId(NetworkGetPlayerIndexFromPed(attacker)) or attacker
			if weaponName then
				TriggerServerEvent('resesetti:removeDurability',attacker,weaponName,weaponGroup)
			end
...
function DegradeHook:DegradeHandler()
	return DegradeHook.resesettiDegradeHandler()
end
local degradeHook = DegradeHook:new()

degradeHook:DegradeHandler()